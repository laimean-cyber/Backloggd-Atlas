...
(async()=>{
 ...
 const data=[];
 for(const slug of ['portal','portal-2','half-life-2'])data.push({path:'/games/'+slug+'/',title:slug,rating:4.5,year:2011,played:true,checked:true,plays:100,averageTimeHours:17,...await igdbDetails('/games/'+slug+'/', '',env)});
 ...
 for(const width of [1440,800,390,320]){
  ...
  assert.equal(await page.locator('.stats').last().locator('.card').count(),5);
  assert.equal(await page.locator('#totalTime').textContent(),'2 days, 3 hours');
  assert.equal(await page.evaluate(()=>{const first=document.querySelector('.stats').getBoundingClientRect(),second=document.querySelector('.stats-secondary').getBoundingClientRect();return Math.abs(first.width-second.width)<1}),true);
