...
(async()=>{
 ...
 await page.route('http://localhost:4174/**',r=>{if(r.request().url().includes('/api/details')){requests++;return r.fulfill({json:{details:data,failed:0}})}return r.fulfill({contentType:'text/html',body:fs.readFileSync('dist/index.html','utf8')})});
