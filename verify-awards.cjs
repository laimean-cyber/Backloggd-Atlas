...
(async()=>{
 ...
 for(const width of [1440,390]){
  ...
  const result=await page.evaluate(()=>{const panel=document.querySelector('.awards-panel'),grid=document.querySelector('.grid');return {overflow:document.documentElement.scrollWidth>innerWidth,width:panel.getBoundingClientRect().width,gridWidth:grid.getBoundingClientRect().width,years:[...document.querySelectorAll('.award-year')].map(x=>+x.textContent),styles:[...document.querySelectorAll('.award-poster')].map(img=>{const s=getComputedStyle(img);return [s.opacity,s.transform,s.outlineWidth,s.outlineColor]}),afterGameModes:panel.previousElementSibling.contains(document.getElementById('modeDonut'))}});
  assert.equal(result.overflow,false);assert.equal(result.width,result.gridWidth);assert.equal(result.afterGameModes,true);assert.deepEqual(result.years,Array.from({length:12},(_,i)=>2014+i));
