...
const trigger=page.locator(selector).first();await trigger.hover();assert.equal(await page.locator('#hoverCard').isVisible(),false);
await trigger.click();assert.equal(await page.locator('#hoverList .hover-game').count(),count);assert(await page.locator('#hoverCard').isVisible());assert.equal(await trigger.getAttribute('aria-expanded'),'true');
await trigger.click();assert.equal(await page.locator('#hoverCard').isVisible(),false);
...
console.log('Passed spread menus: correct games, played filter, click toggle, keyboard, desktop/mobile bounds.');}finally{await browser.close()}})().catch(e=>{console.error(e);process.exit(1)});
