...
(async()=>{
 ...
 for(const width of [1440,390]){
  ...
  assert.equal(await page.locator('.genre-preference-dot').evaluate(el=>el.style.left),'75%');
  assert.equal(await page.locator('.genre-preference-dot').evaluate(el=>el.style.top),'20%');
  assert.deepEqual(await page.locator('.genre-preference-tick').allTextContents(),['0%','25%','50%','75%','100%']);
  assert.deepEqual(await page.locator('.genre-preference-line span').allTextContents(),['0%','20%','40%','60%','80%','100%']);
