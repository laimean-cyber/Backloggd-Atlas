...
(async()=>{
 ...
 for(const width of [1440,390]){
  ...
  assert(Math.abs(await page.locator('.genre-preference-dot').evaluate(el=>parseFloat(el.style.left))-Math.log10(3)*100)<.001);
  assert.equal(await page.locator('.genre-preference-dot').evaluate(el=>el.style.top),'50%');
  assert.deepEqual(await page.locator('.genre-preference-tick').allTextContents(),['1','3','10']);
  assert.deepEqual(await page.locator('.genre-preference-line span').allTextContents(),['3.5','3.75','4.0','4.25','4.5']);
  ...
  await page.evaluate(()=>{games=[{title:'Only',played:true,rating:5,genres:['RPG']}];render()});
  assert.equal(await page.locator('.genre-preference-dot').evaluate(el=>el.style.left),'0%');
  assert.equal(await page.locator('.genre-preference-dot').evaluate(el=>el.style.top),'0%');
